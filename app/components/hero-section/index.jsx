            ...
            <div className="grid grid-cols-1 items-center md:grid-cols-2 lg:gap-12 gap-y-8">
                <div className="flex flex-col items-start justify-center rounded-lg p-3 lg:py-5 lg:px-12 bg-primary-bg h-full">
                    <div className="flex w-full justify-center">
                        <Image src={profile.avatar_url} width={128} height={128} alt={profile.name} className="rounded-full transition-all duration-1000 grayscale hover:grayscale-0 hover:scale-110 cursor-pointer" />
                    </div>
                    <p className="text-gray-300 text-sm lg:text-base my-4 lg:my-6 text-center">{profile.bio}</p>

                    <div className="w-full flex justify-center items-center gap-5">
                        <Link href={userData.github} target="_blank" className="transition-all text-teal-500 hover:scale-125 duration-300">
                            <BsGithub size={24} />
                        </Link>
                        <Link href={userData.linkedIn} target="_blank" className="transition-all text-teal-500 hover:scale-125 duration-300">
                            <BsLinkedin size={24} />
                        </Link>
                        <Link href={userData.facebook} target="_blank" className="transition-all text-teal-500 hover:scale-125 duration-300">
                            <FaFacebook size={24} />
                        </Link>
                        <Link href={userData.leetcode} target="_blank" className="transition-all text-teal-500 hover:scale-125 duration-300">
                            <SiLeetcode size={24} />
                        </Link>
                        <Link href={userData.twitter} target="_blank" className="transition-all text-teal-500 hover:scale-125 duration-300">
                            ...
                            <div className="ml-4 lg:ml-8 mr-2">
                                ...
                                {userData.skills.map((skill, i) => (
                                    <>
                                        ...
                                        {i !== userData.skills.length - 1 && <span className="text-gray-400">{"', '"}</span>}
                                    </>
                                ))}
                                <span className="text-gray-400">{"'],"}</span>
                            </div>
